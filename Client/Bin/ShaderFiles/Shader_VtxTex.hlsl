...
PS_OUT PS_MAIN_AlphaTestGuage(PS_IN In)
{
	PS_OUT			Out = (PS_OUT)0;

	Out.vColor = g_Texture.Sample(LinearSampler, In.vTexUV);

	if (g_Time < In.vTexUV.x)
		discard;
...
PS_OUT PS_MAIN_BARGUAGE(PS_IN In)
{
	...
	Out.vColor.rgb = vDiffuse.rgb * vColor.rgb;
...
PS_OUT PS_MAIN_NoDiffuseColorGuage(PS_IN In)
{
	PS_OUT         Out = (PS_OUT)0;
