#include "stdafx.h"
#include "..\public\Path_Mesh.h"
#include "GameInstance.h"
#include "ControlMove.h"
#include "Interaction_Com.h"

CPath_Mesh::CPath_Mesh(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CEnviromentObj(pDevice, pContext)
{
}

CPath_Mesh::CPath_Mesh(const CPath_Mesh & rhs)
	: CEnviromentObj(rhs)
{
}

HRESULT CPath_Mesh::Initialize_Prototype()
{
...
HRESULT CPath_Mesh::Initialize(void * pArg)
{
...
HRESULT CPath_Mesh::RenderShadow()
{
...
HRESULT CPath_Mesh::Add_AdditionalComponent(_uint iLevelIndex, const _tchar * pComTag, COMPONENTS_OPTION eComponentOption)
{
...
HRESULT CPath_Mesh::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CPath_Mesh::SetUp_ShaderResources()
{
...
HRESULT CPath_Mesh::SetUp_ShadowShaderResources()
{
	NULL_CHECK_RETURN(m_pShaderCom, E_FAIL);
	FAILED_CHECK_RETURN(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix"), E_FAIL);
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CPath_Mesh * CPath_Mesh::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPath_Mesh*		pInstance = new CPath_Mesh(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CPath_Mesh");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CPath_Mesh::Clone(void * pArg)
{
	CPath_Mesh*		pInstance = new CPath_Mesh(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CPath_Mesh");
		Safe_Release(pInstance);
	}
	return pInstance;
}
