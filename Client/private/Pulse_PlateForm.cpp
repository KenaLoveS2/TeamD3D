#include "stdafx.h"
#include "..\public\Pulse_PlateForm.h"
#include "GameInstance.h"
#include "ControlMove.h"
#include "Interaction_Com.h"


CPulse_PlateForm::CPulse_PlateForm(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CEnviromentObj(pDevice, pContext)
{
}

CPulse_PlateForm::CPulse_PlateForm(const CPulse_PlateForm & rhs)
	: CEnviromentObj(rhs)
{
}

HRESULT CPulse_PlateForm::Initialize_Prototype()
{
...
HRESULT CPulse_PlateForm::Initialize(void * pArg)
{
...
void CPulse_PlateForm::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CPulse_PlateForm::Late_Tick(_float fTimeDelta)
{
...
HRESULT CPulse_PlateForm::Render()
{
...
HRESULT CPulse_PlateForm::RenderShadow()
{
...
HRESULT CPulse_PlateForm::Add_AdditionalComponent(_uint iLevelIndex, const _tchar * pComTag, COMPONENTS_OPTION eComponentOption)
{
...
HRESULT CPulse_PlateForm::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CPulse_PlateForm::SetUp_ShaderResources()
{
...
HRESULT CPulse_PlateForm::SetUp_ShadowShaderResources()
{
	NULL_CHECK_RETURN(m_pShaderCom, E_FAIL);
	FAILED_CHECK_RETURN(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix"), E_FAIL);
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CPulse_PlateForm * CPulse_PlateForm::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPulse_PlateForm*		pInstance = new CPulse_PlateForm(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CPulse_PlateForm");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CPulse_PlateForm::Clone(void * pArg)
{
	CPulse_PlateForm*		pInstance = new CPulse_PlateForm(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CPulse_PlateForm");
		Safe_Release(pInstance);
	}
	return pInstance;
}
