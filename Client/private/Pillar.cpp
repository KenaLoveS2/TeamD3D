...
HRESULT CPillar::Initialize_Prototype()
{
...
HRESULT CPillar::Add_AdditionalComponent(_uint iLevelIndex, const _tchar * pComTag, COMPONENTS_OPTION eComponentOption)
{
...
HRESULT CPillar::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CPillar::SetUp_ShaderResources()
{
...
HRESULT CPillar::SetUp_ShadowShaderResources()
{
	NULL_CHECK_RETURN(m_pShaderCom, E_FAIL);
	FAILED_CHECK_RETURN(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix"), E_FAIL);
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CPillar * CPillar::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
