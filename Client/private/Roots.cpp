#include "stdafx.h"
#include "..\public\Roots.h"
#include "GameInstance.h"
#include "ControlMove.h"
#include "Interaction_Com.h"

CRoots::CRoots(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CEnviromentObj(pDevice, pContext)
{
}

CRoots::CRoots(const CRoots & rhs)
	: CEnviromentObj(rhs)
{
}

HRESULT CRoots::Initialize_Prototype()
{
...
HRESULT CRoots::Initialize(void * pArg)
{
...
HRESULT CRoots::Late_Initialize(void* pArg)
{
...
void CRoots::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
}

void CRoots::Late_Tick(_float fTimeDelta)
{
...
HRESULT CRoots::Add_AdditionalComponent(_uint iLevelIndex, const _tchar * pComTag, COMPONENTS_OPTION eComponentOption)
{
...
HRESULT CRoots::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CRoots::SetUp_ShaderResources()
{
...
HRESULT CRoots::SetUp_ShadowShaderResources()
{
	NULL_CHECK_RETURN(m_pShaderCom, E_FAIL);
	FAILED_CHECK_RETURN(m_pTransformCom->Bind_ShaderResource(m_pShaderCom, "g_WorldMatrix"), E_FAIL);
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CRoots * CRoots::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRoots*		pInstance = new CRoots(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CRoots");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CRoots::Clone(void * pArg)
{
	CRoots*		pInstance = new CRoots(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CRoots");
		Safe_Release(pInstance);
	}
	return pInstance;
}
