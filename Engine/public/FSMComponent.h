...
typedef struct ENGINE_DLL tagFSM_State : public CBase
{
	...
	// 이 상태를 반복할 때 실행하는 함수
	std::function<void(_float)> Tick = nullptr;
	// 이 상태를 나갈때 실행하는 함수
...
public:
	...
	CFSMComponentBuilder& Tick(const std::function<void(_float)>& tick)
	{
		assert(m_pBuildState != nullptr);
	...
	template<typename T>
	CFSMComponentBuilder& Tick(T* obj, void (T::*memFunc)(_float))
	{
		assert(m_pBuildState != nullptr);
		assert(m_pBuildState->Tick == nullptr);
		m_pBuildState->Tick = [obj, memFunc](_float TimeDelta)
		{
